# Position Keeper — Developer Guide

For the use-case overview, see [README.md](README.md).

This guide covers building, running, and customizing the positionkeeper example.

Building with Maven
---------------------------
The positionkeeper example can be built using Maven:

```bash
# From the voltdb-examples root directory
mvn clean package -pl positionkeeper -DskipTests

# Or from the positionkeeper directory
cd positionkeeper
...
VOLTDB_IMAGE=voltactivedata/volt-developer-edition:15.2.0_voltdb mvn verify -pl positionkeeper -Pbenchmark -DskipTests
```

This will:
1. Start VoltDB Developer Edition in a Docker container
2. Load the DDL schema and stored procedures
3. Run the PositionsBenchmark client for 60 seconds
4. Shut down cleanly
...
# Build the positionkeeper image
make -C docker build-positionkeeper

# Run (initializes schema + runs benchmark)
docker compose -f positionkeeper/docker-compose.yml up

# Run with custom settings
DURATION=60 docker compose -f positionkeeper/docker-compose.yml up

# Clean up
docker compose -f positionkeeper/docker-compose.yml down -v
```
