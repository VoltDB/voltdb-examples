# Ad Performance — Developer Guide

For the use-case overview, see [README.md](README.md).

This guide covers building, running, and customizing the adperformance example.

Building with Maven
---------------------------
The adperformance example can be built using Maven:

```bash
# From the voltdb-examples root directory
mvn clean package -pl adperformance -DskipTests

# Or from the adperformance directory
cd adperformance
...
mvn verify -pl adperformance -Pbenchmark -DskipTests

# Use a different VoltDB version
VOLTDB_IMAGE=voltactivedata/volt-developer-edition:15.2.0_voltdb mvn verify -pl adperformance -Pbenchmark -DskipTests
```

This will:
1. Start VoltDB Developer Edition in a Docker container
2. Load the DDL schema and stored procedures
3. Run the AdTrackingBenchmark client for 60 seconds
4. Shut down cleanly
...
# Build the adperformance image
make -C docker build-adperformance

# Run (initializes schema + runs benchmark)
docker compose -f adperformance/docker-compose.yml up

# Run with custom settings
DURATION=60 docker compose -f adperformance/docker-compose.yml up

# Clean up
docker compose -f adperformance/docker-compose.yml down -v
```
