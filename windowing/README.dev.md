# Windowing — Developer Guide

For the use-case overview, see [README.md](README.md).

This guide covers building, running, and customizing the windowing example.

Building with Maven
---------------------------
The windowing example can be built using Maven:

```bash
# From the voltdb-examples root directory
mvn clean package -pl windowing -DskipTests

# Or from the windowing directory
cd windowing
...
mvn verify -pl windowing -Pbenchmark -DskipTests

# Use a different VoltDB version
VOLTDB_IMAGE=voltactivedata/volt-developer-edition:15.2.0_voltdb mvn verify -pl windowing -Pbenchmark -DskipTests
```

This will:
1. Start VoltDB Developer Edition in a Docker container
2. Load the DDL schema and stored procedures
3. Run the WindowingApp client for 60 seconds
4. Shut down cleanly
...
# Build the windowing image
make -C docker build-windowing

# Run (initializes schema + runs benchmark)
docker compose -f windowing/docker-compose.yml up

# Run with custom settings
DURATION=60 docker compose -f windowing/docker-compose.yml up

# Clean up
docker compose -f windowing/docker-compose.yml down -v
```
...
The default settings for the client have it keep 30 seconds worth of tuples, deleting older tuples as an ongoing process. See the section below on *Client options* for how to run in other modes.
