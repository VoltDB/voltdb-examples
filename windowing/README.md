...
The default settings keep 30 seconds worth of tuples, deleting older tuples as an ongoing process. The app supports several modes — by row count target (`maxrows`), by age (`historyseconds`), and inline vs. non-inline deleting — which control how the windowing behavior is enforced.
