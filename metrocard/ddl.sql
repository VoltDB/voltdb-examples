...
-------------- REPLICATED TABLES ------------------------------------------------
CREATE TABLE mc_stations (
  station_id            SMALLINT          NOT NULL,
  name                  VARCHAR(25 BYTES) NOT NULL,
  fare                  SMALLINT          DEFAULT 250 NOT NULL,
  weight                INTEGER           NOT NULL,
  CONSTRAINT PK_mc_stations PRIMARY KEY (station_id)
);

-------------- PARTITIONED TABLES -----------------------------------------------
CREATE TABLE mc_cards(
  ...
  CONSTRAINT PK_mc_cards_card_id PRIMARY KEY ( card_id )
);
PARTITION TABLE mc_cards ON COLUMN card_id;
...
PARTITION TABLE mc_activity ON COLUMN card_id;

CREATE STREAM mc_card_alert_export PARTITION ON COLUMN card_id EXPORT TO TARGET mc_alertstream (
  card_id               INTEGER        NOT NULL,
...
-------------- VIEWS ------------------------------------------------------------
CREATE VIEW mc_secondly_entries_by_station
AS
SELECT
...
CREATE PROCEDURE PARTITION ON TABLE mc_cards COLUMN card_id PARAMETER 0 FROM CLASS metrocard.McCardSwipe;
...
CREATE PROCEDURE mc_ReplenishCard PARTITION ON TABLE mc_cards COLUMN card_id PARAMETER 1 AS
UPDATE mc_cards SET balance = balance + ?
