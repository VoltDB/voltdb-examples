# MetroCard — Developer Guide

For the use-case overview, see [README.md](README.md).

This guide covers building, running, and customizing the metrocard example.

Building with Maven
---------------------------
The metrocard example can be built using Maven:

```bash
# From the voltdb-examples root directory
mvn clean package -pl metrocard -DskipTests

# Or from the metrocard directory
cd metrocard
...
mvn verify -pl metrocard -Pbenchmark -DskipTests

# Use a different VoltDB version
VOLTDB_IMAGE=voltactivedata/volt-developer-edition:15.2.0_voltdb mvn verify -pl metrocard -Pbenchmark -DskipTests
```

This will:
1. Start VoltDB Developer Edition in a Docker container
2. Load the DDL schema and stored procedures
3. Load station data from CSV
4. Run the MetroBenchmark client for 60 seconds
5. Shut down cleanly
...
# Build the metrocard image
make -C docker build-metrocard

# Run (initializes schema + runs benchmark)
docker compose -f metrocard/docker-compose.yml up

# Run with custom settings
DURATION=60 docker compose -f metrocard/docker-compose.yml up

# Clean up
docker compose -f metrocard/docker-compose.yml down -v
```
