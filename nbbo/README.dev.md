# NBBO — Developer Guide

For the use-case overview, see [README.md](README.md).

This guide covers building, running, and customizing the NBBO example.

Building with Maven
---------------------------
The NBBO example can be built using Maven:

```bash
# From the voltdb-examples root directory
mvn clean package -pl nbbo -DskipTests

# Or from the nbbo directory
cd nbbo
...
VOLTDB_IMAGE=voltactivedata/volt-developer-edition:15.2.0_voltdb mvn verify -pl nbbo -Pbenchmark -DskipTests
```

This will:
1. Start VoltDB Developer Edition in a Docker container
2. Load the DDL schema and stored procedures
3. Run the NbboBenchmark client for 60 seconds
4. Shut down cleanly
...
# Build the nbbo image
make -C docker build-nbbo

# Run (initializes schema + runs benchmark)
docker compose -f nbbo/docker-compose.yml up

# Run with custom settings
DURATION=60 docker compose -f nbbo/docker-compose.yml up

# Clean up
docker compose -f nbbo/docker-compose.yml down -v
```
