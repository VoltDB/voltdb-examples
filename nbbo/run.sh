...
jars() {
    ../mvnw package -f ../pom.xml -pl nbbo -DskipTests
    # copy jars to module root for compatibility with traditional run.sh usage
    cp target/nbbo-1.0-SNAPSHOT.jar nbbo-procs.jar
    cp target/nbbo-1.0-SNAPSHOT.jar nbbo-client.jar
}
...
client_help() {
    jars_ifneeded
    java $JAVA_OPTS -classpath nbbo-client.jar:$CLIENTCLASSPATH nbbo.NbboBenchmark --help
}

# run the client that drives the example with some editable options
client() {
    jars_ifneeded
    java $JAVA_OPTS -classpath nbbo-client.jar:$CLIENTCLASSPATH nbbo.NbboBenchmark \
        --displayinterval=5 \
        --warmup=5 \
        --duration=120 \
        --ratelimit=20000 \
