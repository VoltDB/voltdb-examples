# Call Center — Developer Guide

For the use-case overview, see [README.md](README.md).

This guide covers building, running, and customizing the callcenter example.

Building with Maven
---------------------------
The callcenter example can be built using Maven:

```bash
# From the voltdb-examples root directory
mvn clean package -pl callcenter -DskipTests

# Or from the callcenter directory
cd callcenter
...
mvn verify -pl callcenter -Pbenchmark -DskipTests

# Use a different VoltDB version
VOLTDB_IMAGE=voltactivedata/volt-developer-edition:15.2.0_voltdb mvn verify -pl callcenter -Pbenchmark -DskipTests
```

This will:
1. Start VoltDB Developer Edition in a Docker container
2. Load the DDL schema and stored procedures
3. Run the CallCenterClient for 60 seconds
4. Shut down cleanly
...
# Build the callcenter image
make -C docker build-callcenter

# Run (initializes schema + runs benchmark)
docker compose -f callcenter/docker-compose.yml up

# Run with custom settings
DURATION=60 docker compose -f callcenter/docker-compose.yml up

# Clean up
docker compose -f callcenter/docker-compose.yml down -v
```
