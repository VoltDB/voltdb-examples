...
jars() {
    ../mvnw package -f ../pom.xml -pl callcenter -DskipTests
    # copy jars to module root for compatibility with traditional run.sh usage
    cp target/callcenter-1.0-SNAPSHOT.jar callcenter-procs.jar
    cp target/callcenter-1.0-SNAPSHOT.jar callcenter-client.jar
}
...
client_help() {
    jars_ifneeded
    java $JAVA_OPTS -classpath callcenter-client.jar:$CLIENTCLASSPATH callcenter.CallCenterApp --help
}

# run the client that drives the example with some editable options
client() {
    jars_ifneeded
    java $JAVA_OPTS -classpath callcenter-client.jar:$CLIENTCLASSPATH callcenter.CallCenterApp \
        --displayinterval=5 \
        --duration=120 \
        --servers=$SERVERS \
        --agents=3000 \
