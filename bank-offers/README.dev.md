# Bank Offers — Developer Guide

For the use-case overview, see [README.md](README.md).

This guide covers building, running, and customizing the bank-offers example.

Building with Maven
---------------------------
The bank-offers example can be built using Maven:

```bash
# From the voltdb-examples root directory
mvn clean package -pl bank-offers -DskipTests

# Or from the bank-offers directory
cd bank-offers
...
mvn verify -pl bank-offers -Pbenchmark -DskipTests

# Use a different VoltDB version
VOLTDB_IMAGE=voltactivedata/volt-developer-edition:15.2.0_voltdb mvn verify -pl bank-offers -Pbenchmark -DskipTests
```

This will:
1. Start VoltDB Developer Edition in a Docker container
2. Load the DDL schema and stored procedures
3. Pre-populate customer and vendor data
4. Run the OffersBenchmark client for 60 seconds
5. Shut down cleanly
...
# Build the bank-offers image
make -C docker build-bank-offers

# Run (initializes schema + runs benchmark)
docker compose -f bank-offers/docker-compose.yml up

# Run with custom settings
DURATION=60 docker compose -f bank-offers/docker-compose.yml up

# Clean up
docker compose -f bank-offers/docker-compose.yml down -v
```
