...
-------------- REPLICATED TABLES ------------------------------------------------
CREATE TABLE fd_stations (
  station_id            SMALLINT          NOT NULL,
  name                  VARCHAR(25 BYTES) NOT NULL,
  fare                  SMALLINT          DEFAULT 250 NOT NULL,
  weight                INTEGER           NOT NULL,
  CONSTRAINT PK_fd_stations PRIMARY KEY (station_id)
);

CREATE TABLE fd_trains (
  train_id            SMALLINT          NOT NULL,
  name                VARCHAR(25) NOT NULL,
  CONSTRAINT PK_fd_trains PRIMARY KEY (train_id)
);

CREATE TABLE fd_wait_time (
  station_id          SMALLINT  NOT NULL,
...
PARTITION TABLE fd_wait_time ON COLUMN station_id;
-------------- PARTITIONED TABLES -----------------------------------------------
CREATE TABLE fd_cards(
  ...
  CONSTRAINT PK_fd_cards_card_id PRIMARY KEY ( card_id )
);
PARTITION TABLE fd_cards ON COLUMN card_id;

CREATE TABLE fd_activity(
  card_id               INTEGER        NOT NULL,
...
PARTITION TABLE fd_activity ON COLUMN card_id;
CREATE UNIQUE INDEX fd_aCardDate ON fd_activity (card_id, date_time);
CREATE UNIQUE INDEX fd_aStationDateCard ON fd_activity (station_id, date_time, card_id);

CREATE TABLE fd_train_activity(
...
PARTITION TABLE fd_train_activity ON COLUMN station_id;
CREATE INDEX fd_taStationDepart ON fd_train_activity (station_id, time);

CREATE STREAM fd_card_alert_export PARTITION ON COLUMN card_id EXPORT TO TARGET fd_alertstream (
  card_id               INTEGER        NOT NULL,
...
CREATE STREAM fd_update_requests PARTITION ON COLUMN station_id EXPORT TO TARGET fd_updatewaittime (
  station_id SMALLINT NOT NULL,
  ttl        INTEGER DEFAULT 3000000 -- 5 min
);
-------------- VIEWS ------------------------------------------------------------
CREATE VIEW fd_secondly_entries_by_station
AS
SELECT
...
CREATE VIEW fd_wait_time_by_station
AS
SELECT
  station_id,
...
CREATE VIEW fd_secondly_card_acceptance_rate
AS
SELECT
  TRUNCATE(SECOND, date_time) AS second,
  accept,
...
CREATE PROCEDURE PARTITION ON TABLE fd_cards COLUMN card_id PARAMETER 0 FROM CLASS frauddetection.FdCardSwipe;
CREATE PROCEDURE FROM CLASS frauddetection.UpdateWaitTime;
CREATE PROCEDURE PARTITION ON TABLE fd_train_activity COLUMN station_id PARAMETER 0 FROM CLASS frauddetection.UpdateWaitTimeForStation;
CREATE PROCEDURE PARTITION ON TABLE fd_train_activity COLUMN station_id PARAMETER 1 FROM CLASS frauddetection.TrainActivity;
CREATE PROCEDURE FROM CLASS frauddetection.GetCardAcceptanceRate;

CREATE PROCEDURE fd_ReplenishCard PARTITION ON TABLE fd_cards COLUMN card_id PARAMETER 1 AS
UPDATE fd_cards SET balance = balance + ?
WHERE card_id = ? AND card_type = 0;

CREATE PROCEDURE fd_GetBusiestStationInLastMinute AS
SELECT s.name, SUM(v.activities) as swipes, SUM(v.entries) as entries
FROM fd_secondly_entries_by_station v
INNER JOIN fd_stations s ON s.station_id = v.station_id
WHERE
  v.second >= DATEADD(SECOND, -60, NOW)
GROUP BY s.name
ORDER BY swipes DESC;

CREATE PROCEDURE fd_GetStationWaitTime AS
SELECT s.name as name, w.total_time as totaltime, w.entries as entries
FROM fd_wait_time_by_station w
INNER JOIN fd_stations s ON s.station_id = w.station_id
ORDER BY name;

CREATE PROCEDURE fd_GetSwipesPerSecond AS
SELECT second, activities, entries
FROM fd_secondly_stats
WHERE
