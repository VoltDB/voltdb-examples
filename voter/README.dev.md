# Voter — Developer Guide

For the use-case overview, see [README.md](README.md).

This guide covers building, running, and customizing the voter example.

Building with Maven
---------------------------
The voter example can be built using Maven:

```bash
# From the voltdb-examples root directory
mvn clean package -pl voter -DskipTests

# Or from the voter directory
cd voter
...
### Option 1: Maven Benchmark Profile (Recommended for Development)

The easiest way to run the benchmark locally is using Docker with the Maven benchmark profile:
...
VOLTDB_IMAGE=voltactivedata/volt-developer-edition:15.2.0_voltdb mvn verify -pl voter -Pbenchmark -DskipTests
```

This will:
1. Start VoltDB Developer Edition in a Docker container
2. Load the DDL schema and stored procedures
3. Initialize contestants
4. Run the AsyncBenchmark client for 60 seconds
5. Shut down cleanly

### Option 2: Docker Compose (Recommended for Local Testing)

Run the voter benchmark against a local VoltDB container:
...
DURATION=60 CONTESTANTS=6 docker compose -f voter/docker-compose.yml up

# Initialize schema only (no benchmark)
MODE=init docker compose -f voter/docker-compose.yml up

# Run benchmark only (schema must exist)
MODE=run docker compose -f voter/docker-compose.yml up

# Clean up
docker compose -f voter/docker-compose.yml down -v
```

See [docker/README.md](../docker/README.md) for more details on Docker Compose options.

### Option 3: Standalone Docker Image

Build and run the voter container independently:

```bash
# Build from repo root
docker build -t voltdb-example-voter -f voter/Dockerfile .

# Run against an existing VoltDB cluster
docker run --rm \
  -e VOLTDB_SERVERS=voltdb-host:21212 \
  -e MODE=init-run \
  -e DURATION=120 \
  -e CONTESTANTS=6 \
  -e MAX_VOTES=2 \
  voltdb-example-voter
```
...
|----------|---------|-------------|
| `MODE` | `init-run` | `init` (load schema), `run` (benchmark only), or `init-run` (both) |
| `VOLTDB_SERVERS` | `localhost:21212` | VoltDB cluster connection string |
| `DURATION` | `120` | Benchmark duration in seconds |
| `WARMUP` | `5` | Warmup period in seconds |
| `CONTESTANTS` | `6` | Number of contestants (1-12) |
| `MAX_VOTES` | `2` | Maximum votes per phone number |
